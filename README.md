...
- **순현금(Net Cash) = 유동자금 − 차입금(사채 포함)**
  - 유동자금 = 현금및현금성자산 + 단기금융상품 + 단기상각후원가금융자산 + 단기당기손익-공정가치금융자산
  - 차입금(사채 포함) = 단기차입금 + 유동성장기부채(없으면 유동성장기차입금 + 유동성사채) + 사채 + 장기차입금
- 분모는 OpenDART `stockTotqySttus`의 `distb_stock_co`(Ⅵ. 유통주식수, 보통주 우선)만 사용하며 값이 없으면 `N/A`로 표시한다.
